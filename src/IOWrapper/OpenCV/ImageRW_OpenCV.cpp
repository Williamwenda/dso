...
{
	cv::Mat m = cv::imread(filename, cd::IMAGE_GRAYSCALE);
	if(m.rows*m.cols==0)
	{
...
{
	cv::Mat m = cv::imread(filename, cv::IMAGE_COLOR);
	if(m.rows*m.cols==0)
	{
		printf("cv::imread could not read image %s! this may segfault. \n", filename.c_str());
		return 0;
...
{
	cv::Mat m = cv::imread(filename, cv::IMAGE_UNCHANGED);
	if(m.rows*m.cols==0)
	{
		printf("cv::imread could not read image %s! this may segfault. \n", filename.c_str());
		return 0;
...
{
	cv::Mat m = cv::imdecode(cv::Mat(numBytes,1,CV_8U, data), cv::IMAGE_GRAYSCALE);
	if(m.rows*m.cols==0)
	{
